    ...
    <div className="bg-card border border-border/50 rounded-3xl p-6 shadow-sm transition-colors duration-300">
            ...
            <div className="flex flex-col items-center">
              ...
              {dailyLimit > 0 && (
                <div className={cn(
                  ...
                  {isOverLimit ? <><AlertCircle className="w-3 h-3" /> OVER LIMIT</> : <><CheckCircle2 className="w-3 h-3" /> ON TRACK</>}
                </div>
              )}
            </div>
          </CircularProgress>
      ...
      <div className="grid grid-cols-3 gap-4 mt-10 pt-6 border-t border-border/50">
        ...
        <div className="flex flex-col items-center text-center border-x border-border/50">
          <div className="bg-yellow-50 dark:bg-yellow-900/20 p-2 rounded-full mb-2"><Zap className="w-4 h-4 text-yellow-500" /></div>
          <span className="text-lg font-bold text-foreground">{nicotineUsed.toFixed(1)}mg</span>
          <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">NICOTINE</span>
        </div>
        <div className="flex flex-col items-center text-center">
