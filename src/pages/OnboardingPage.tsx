...
function GoalsStep({ onSubmit, onBack }: { onSubmit: (data: GoalsForm) => void, onBack: () => void }) {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<GoalsForm>({
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="pt-4">
          <Button type="submit" className="w-full h-12 text-lg bg-gradient-to-r from-sky-500 to-violet-600 hover:opacity-90 transition-opacity text-white rounded-full shadow-lg shadow-sky-500/20" disabled={isSubmitting}>
